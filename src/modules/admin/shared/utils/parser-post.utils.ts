...
const REGEX_PATTERNS = {
  ...
  CATEGORY: /<span[^>]*class=["'][^"']*mh-meta-categories[^"']*["'][^>]*>([\s\S]*?)<\/span>/i,
  CATEGORY_LINK: /<a[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/i,
  ...
  CHAPTER: /<p[^>]*>([\s\S]*?)<\/p>/gi,
  RAPIDGATOR_ROW:
    /<tr[^>]*>[\s\S]*?<td[^>]*>([^<]*RapidGator[^<]*)<\/td>[\s\S]*?<td[^>]*>[\s\S]*?<a[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>[\s\S]*?<\/td>[\s\S]*?<\/tr>/gi,
...
export function parseCategory(html: string): CategoryInfo | null {
  const match = html.match(REGEX_PATTERNS.CATEGORY);
  if (!match) {
    return null;
  }

  const categoryContent = match[1];
  const linkMatch = categoryContent.match(REGEX_PATTERNS.CATEGORY_LINK);
  ...
  const title = removeHtmlTags(linkMatch[2]).trim();

  if (!href || !title) {
    return null;
  }

  const categorySlugMatch = href.match(/\/category\/(.+)$/i);
  const slug = categorySlugMatch ? categorySlugMatch[1] : '';
...
export function parseThumbnailUrl(html: string): string {
  const match = html.match(REGEX_PATTERNS.THUMBNAIL);

  return match ? normalizeUrl(match[1]) : '';
}

export function parseDateFromMMDDYYYY(dateText: string): Date | null {
...
export function parseLastUpdated(html: string): Date | null {
...
export function parsePostInfoFromHtml(html: string): PostInfo | null {
  try {
    ...
    return {
      ...
      isRead: false,
      category: parseCategory(html) || undefined,
    ...
    if (chapterMatches.length === 0) {
      ...
      while ((rapidGatorMatch = REGEX_PATTERNS.RAPIDGATOR_ROW.exec(contentHtml)) !== null) {
        ...
        if (
          ...
          linkText &&
          storyTitle &&
          storyTitle.includes('RapidGator') &&
          !storyTitle.includes('Premium')
        ) {
          stories.push({
            name: linkText,
            media: mediaUrl,
      ...
      for (let i = 0; i < chapterMatches.length; i++) {
        ...
        const nextChapterStartIndex =
          i + 1 < chapterMatches.length ? chapterMatches[i + 1].startIndex : entryContentEndInHtml;
