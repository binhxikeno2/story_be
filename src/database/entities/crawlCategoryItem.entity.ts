...
import { CrawlCategoryEntity } from './crawlCategory.entity';

@Entity('crawl_category_item')
@Index(['processId', 'pageNo'], { unique: true })
@Index(['processId', 'status'])
export class CrawlCategoryItemEntity extends BaseEntity {
    @Column({ type: 'bigint', name: 'process_id' })
    public processId: number;

    @ManyToOne(() => CrawlCategoryEntity, { onDelete: 'CASCADE' })
    @JoinColumn({ name: 'process_id' })
    public process: CrawlCategoryEntity;
